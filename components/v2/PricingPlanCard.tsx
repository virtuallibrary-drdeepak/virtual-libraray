...
export function PricingPlanCard({
  ...
  const monthlyLabel = formatMonthlyPrice(plan)

  return (
    <div
      className={cn(
        'group relative flex min-h-full w-full flex-col border bg-white shadow-[0_22px_54px_rgba(109,40,217,0.08)] transition-all duration-300 ease-out will-change-transform hover:-translate-y-2 hover:scale-[1.015] hover:shadow-[0_34px_80px_rgba(109,40,217,0.18)] motion-reduce:transform-none motion-reduce:transition-none',
        compact ? 'rounded-[20px] p-4' : 'rounded-[30px] p-6',
        !compact && (tilt === 'left' ? 'hover:-rotate-1' : 'hover:rotate-1'),
        meta.highlight
    ...
    >
      {meta.badge && (
        <span className={cn(
          'absolute top-0 -translate-y-1/2 bg-[#7c3aed] font-bold uppercase tracking-normal text-white shadow-[0_14px_28px_rgba(109,40,217,0.22)]',
          compact ? 'right-4 rounded-full px-3 py-1 text-[10px]' : 'right-5 rounded-xl px-4 py-2 text-xs'
        )}>
          {meta.badge}
        </span>
      )}
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          <h3 className={cn('font-bold tracking-normal text-slate-950', compact ? 'text-xl' : 'text-2xl')}>
            {formatPlanName(plan)}
          </h3>
          <p className={cn('font-medium text-[#5a5d78]', compact ? 'mt-1 text-xs' : 'mt-2 text-sm')}>
            {compact ? `${monthlyLabel} · billed once` : `Valid for ${durationLabel}`}
          </p>
        </div>

        <SelectionCheck active={meta.highlight} compact={compact} />
      </div>

      <div className={cn(compact ? 'mt-4 flex items-end justify-between gap-3' : 'mt-6')}>
        <span className={cn('font-bold tracking-normal text-slate-950', compact ? 'text-3xl' : 'text-5xl')}>
          {formatCurrency(plan.amountPaise, plan.currency)}
        </span>
        {!compact && <span className="ml-2 text-base font-semibold text-[#6b7280]">/ {durationLabel}</span>}
        {compact && (
          <button
            type="button"
            onClick={() => onSelect(plan)}
            className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#f4efff] text-[#6d35df] transition group-hover:bg-[#7c3aed] group-hover:text-white"
            aria-label={`Start with ${durationLabel}`}
          >
...
function formatMonthlyPrice(plan: BillingPlan) {
  const monthlyAmount = Math.round(plan.amountPaise / Math.max(plan.durationMonths, 1))
  return `${formatCurrency(monthlyAmount, plan.currency).replace(/\.00$/, '')}/mo`
}
...
function SelectionCheck({ active, compact = false }: { active: boolean; compact?: boolean }) {
  return (
    <span
      className={cn(
...
function ArrowRightIcon({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 20 20" fill="none" aria-hidden="true" className={className}>
      <path d="M7.5 4.5L12.5 10l-5 5.5" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.8" />
    </svg>
  )
}
